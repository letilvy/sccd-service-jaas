...
var oServer = HTTP.createServer(function(request, response){
    ...
    request.on("end", function(){
        ...
        if(sEntitySet === "KpiSet"){
            var sSqlTotalProject = "select count(*) totalProjects from Project where type='UI5'";
            var sSqlTop3Active = "select top.projectId, top.projectName, top.commit_times, top.tcid, ut.passed, ut.failed, ut.passed+ut.failed totalUT, ut.branch, ut.type from ( select u.pid projectId, p.name projectName, count(*) commit_times, max(u.tcid) tcid from UT u, Project p where DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= date(timestamp) and u.pid = p.pid and u.type = 'UI5' and u.branch = 'master' group by u.pid, p.name ) top, UT ut where top.tcid = ut.tcid order by commit_times desc limit 3;";
            var sSqlHealthyUTProject = "select * from (select count(*) UT_TotalProject from (select t.pid, substring_index(group_concat(t.tcid), ',', -1) last_commit_id from (select u.tcid,   u.pid,   u.timestamp from UT u where timestamp > 0 and branch = 'master' and type = 'UI5' order by u.pid asc, timestamp desc) t group by t.pid) l left join UT ut on l.last_commit_id = ut.tcid) total,  (select count(*) UT_FailedProject from (select t.pid, substring_index(group_concat(t.tcid), ',', -1) last_commit_id from (select u.tcid,   u.pid,   u.timestamp from UT u where timestamp > 0 and branch = 'master' and type = 'UI5' order by u.pid asc, timestamp desc) t group by t.pid) l left join UT ut on l.last_commit_id = ut.tcid where ut.failed > 0) failed,  (select count(*) UT_PassedProject from (select t.pid, substring_index(group_concat(t.tcid), ',', -1) last_commit_id from (select u.tcid,   u.pid,   u.timestamp from UT u where timestamp > 0 and branch = 'master' and type = 'UI5' order by u.pid asc, timestamp desc) t group by t.pid) l left join UT ut on l.last_commit_id = ut.tcid where ut.failed = 0) passed";
            var sSqlHealthyITProject = "select * from (select count(*) IT_TotalProject from (select i.pid, substring_index(group_concat(i.tcid), ',', -1) last_commit_id from (select i.tcid, i.pid, i.timestamp from IT i where timestamp > 0 and branch = 'master' and type='UI5' order by i.pid asc, i.timestamp desc) i group by i.pid) l left join IT it on l.last_commit_id = it.tcid) total, (select count(*) IT_FailedProject from (select i.pid, substring_index(group_concat(i.tcid), ',', -1) last_commit_id from (select i.tcid, i.pid, i.timestamp from IT i where timestamp > 0 and branch = 'master' and type='UI5' order by i.pid asc, i.timestamp desc) i group by i.pid) l left join IT it on l.last_commit_id = it.tcid where it.failed > 0) failed, (select count(*) IT_PassedProject from (select i.pid, substring_index(group_concat(i.tcid), ',', -1) last_commit_id from (select i.tcid, i.pid, i.timestamp from IT i where timestamp > 0 and branch = 'master' and type='UI5' order by i.pid asc, i.timestamp desc) i group by i.pid) l left join IT it on l.last_commit_id = it.tcid where it.failed = 0) passed";
            ...
            switch(sEntitySet){
                ...
                case "ITSet":
                     ...
                     */
                    if(!oUrlParam.pid){
                        sQuery = "SELECT it.tcid AS tcid, it.pid AS projectId, p.name AS projectName, p.team AS teamId, t.name AS teamName, it.passed AS passed, it.failed AS failed, it.assertion AS assertion, it.skipped, it.branch AS branch, it.timestamp AS timestamp, it.branch, it.type FROM ( SELECT i.pid , substring_index(GROUP_CONCAT(i.tcid), ',', -1) AS last_commit_id FROM ( SELECT i.tcid, i.pid, i.timestamp FROM IT i WHERE timestamp > 0 AND branch = 'master' AND type = 'UI5' ORDER BY i.pid ASC, i.timestamp DESC ) i GROUP BY i.pid ) l LEFT JOIN IT it ON l.last_commit_id = it.tcid LEFT JOIN Project p ON it.pid = p.pid LEFT JOIN Team t ON p.team = t.tid ORDER BY p.team DESC, it.assertion DESC";
                    }else{
                        sQuery = "SELECT it.tcid AS tcid, it.pid AS projectId, p.name AS projectName, it.passed AS passed, it.failed AS failed, it.passed + it.failed AS allTestCases, it.assertion AS assertion, it.skipped, p.team AS teamId, t.name AS teamName, it.timestamp AS timestamp, it.branch, it.type FROM IT it LEFT JOIN Project p ON it.pid = p.pid LEFT JOIN Team t ON p.team = t.tid WHERE it.pid = '" + oUrlParam.pid + "' AND it.branch = 'master' AND it.type = 'UI5' ORDER BY timestamp DESC LIMIT 30";
                    }
                    break;
