            ...
            switch(sEntitySet){
                ...
                case "UTSet":
                    ...
                    if(!oUrlParam.pid){
                        ...
                        sQuery = "select ut.tcid tcid, ut.pid projectId, p.name projectName, p.team teamId, t.name teamName, ut.passed passed, ut.failed failed, ut.assertion assertion, ut.skipped, ut.inclstmtlines includedLine, ut.inclstmtcover includedCover, round(ut.inclstmtlines * ut.inclstmtcover) includedCoverLine, ut.inclstmtlines - round(ut.inclstmtlines * ut.inclstmtcover) notIncludedCoverLine, ut.allstmtlines allLine, ut.allstmtcover allCover, round(ut.allstmtlines * ut.allstmtcover) allCoverLine,  ut.allstmtlines - ut.inclstmtlines notIncludedLine, ut.timestamp timestamp from (select t.pid, substring_index(group_concat(t.tcid), ',', -1) last_commit_id from (select u.tcid, u.pid, u.timestamp from UT u where timestamp > 0 and branch = 'master' order by u.pid asc, timestamp desc) t group by t.pid) l left join UT ut on l.last_commit_id = ut.tcid left join Project p on ut.pid = p.pid left join Team t on p.team = t.tid order by p.team desc, ut.assertion desc";
                    }else{
                ...
                case "ITSet":
                     ...
                     */
                    if(!oUrlParam.pid){
                        sQuery = "select  it.tcid tcid, it.pid projectId, p.name projectName, p.team teamId, t.name teamName, it.passed passed, it.failed failed, it.assertion assertion, it.skipped, it.branch branch, it.timestamp timestamp from (select  i.pid,  substring_index(group_concat(i.tcid), ',', -1) last_commit_id from  (select      i.tcid,     i.pid,     i.timestamp     from IT i     where timestamp > 0     and branch = 'master'     order by i.pid asc, i.timestamp desc) i group by i.pid) l left join IT it on l.last_commit_id = it.tcid left join Project p on it.pid = p.pid left join Team t on p.team = t.tid order by p.team desc, it.assertion desc";
                    }else{
                        sQuery = "select it.tcid tcid, it.pid projectId, p.name projectName, it.passed passed, it.failed failed, it.passed + it.failed allTestCases, it.assertion assertion, it.skipped, p.team teamId, t.name teamName, it.timestamp timestamp from IT it left join Project p on (it.pid = p.pid ) left join Team t on (p.team = t.tid) where it.pid = '" + oUrlParam.pid + "' and it.branch='master' order by timestamp desc limit 30;"
                    }
                    break;
