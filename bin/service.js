...
var oServer = HTTP.createServer(function(request, response){
    ...
    request.on("end", function(){
        ...
        select 
            ...
            ut.teamId,
            ut.teamName,
            ut.passed+ut.failed totalUT,
            ut.assertion UTAssertion,
            it.passed+ut.failed totalIT,
            it.assertion ITAssertion
        from
                (select 
                    ut.tcid tcid,
                    ut.pid projectId,
        ...
        left join
                ...
                from
                    (select 
                        i.pid, 
                        substring_index(group_concat(i.tcid), ',', -1) last_commit_id
                    from 
                        (select 
                            i.tcid,
                            i.pid,
                        ...
                        where timestamp > 0
                        and branch = 'master'
                        order by i.pid asc, i.timestamp desc) i group by i.pid) l
                left join IT it
                on l.last_commit_id = it.tcid
                left join Project p
                on it.pid = p.pid
                left join Team t
                on p.team = t.tid) it
        on ut.projectId = it.projectId
        order by  ut.teamId desc, ut.assertion desc, it.assertion desc
        */
        ...
        /*
            ...
            left join Team t
            on p.team = t.tid
            order by p.team desc, ut.assertion desc
        */
        ...
        /*
            ...
            left join Team t
            on (p.team = t.tid)
            where ut.pid = 'sap.support.sae' 
            and ut.branch='master'
        ...
        */
        ...
        /*
            ...
            left join Team t
            on p.team = t.tid
            order by p.team desc, it.assertion desc
        */
        ...
        left join Team t
        on (p.team = t.tid)
        where it.pid = 'sap.support.sae' 
        and it.branch='master'
        order by timestamp desc
        */
